...
export default function MagicRings({
  ...
  propsRef.current = {
    ...
    fadeOut, followMouse, mouseInfluence, hoverScale, parallax, clickBurst,
  };
  ...
  useEffect(() => {
    ...
    const uniforms = {
      ...
      uResolution: { value: new THREE.Vector2() }, uColor: { value: new THREE.Color() },
      uColorTwo: { value: new THREE.Color() }, uLineThickness: { value: 0 },
      uBaseRadius: { value: 0 }, uRadiusStep: { value: 0 }, uScaleRate: { value: 0 },
      uRingCount: { value: 0 }, uOpacity: { value: 1 }, uNoiseAmount: { value: 0 },
      uRotation: { value: 0 }, uRingGap: { value: 1.6 }, uFadeIn: { value: 0.5 },
      uFadeOut: { value: 0.75 }, uMouse: { value: new THREE.Vector2() },
    ...
    const onMouseMove = (event: MouseEvent) => {
      ...
      mouseRef.current = [(event.clientX - rect.left) / rect.width - 0.5, -((event.clientY - rect.top) / rect.height - 0.5)];
    };
    ...
    const animate = (time: number) => {
      ...
      if (!visible || document.hidden || !propsRef.current) return;
      const props = propsRef.current;
      smoothMouseRef.current[0] += (mouseRef.current[0] - smoothMouseRef.current[0]) * 0.08;
      smoothMouseRef.current[1] += (mouseRef.current[1] - smoothMouseRef.current[1]) * 0.08;
      hoverAmountRef.current += ((isHoveredRef.current ? 1 : 0) - hoverAmountRef.current) * 0.08;
      burstRef.current = burstRef.current < 0.001 ? 0 : burstRef.current * 0.95;

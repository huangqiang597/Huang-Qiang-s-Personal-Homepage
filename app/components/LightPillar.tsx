...
export default function LightPillar({
  ...
  useEffect(() => {
    ...
    const isLowEndDevice = isMobile || (navigator.hardwareConcurrency || 8) <= 4;
    ...
    const handleMouseMove = (event: MouseEvent) => {
      if (!interactive || mouseMoveTimeout !== null) return;
    ...
    const renderFrame = (currentTime: number) => {
      ...
      if (!document.hidden && deltaTime >= frameTime) {
    ...
    renderer.render(scene, camera);
    if (!reduceMotion) rafRef.current = requestAnimationFrame(renderFrame);
    ...
    return () => {
      ...
      if (resizeTimeout !== null) window.clearTimeout(resizeTimeout);
      if (mouseMoveTimeout !== null) window.clearTimeout(mouseMoveTimeout);
      geometry.dispose();
      material.dispose();
      renderer.dispose();
  ...
  if (!webGLSupported) {
    return <div className={`light-pillar-fallback ${className}`} aria-hidden="true" />;
  }
